...
- [ ] T050 [P] Run `uv run ruff check --fix` on all new backend handler files (`_auth.py`, `list_batches.py`, `create_batch.py`, `update_batch.py`, `update_batch_status.py`, `get_me.py`, `close_expired_batches.py`) and commit any lint fixes — NOTE: ruff is not installed in this project; skip or add ruff as a dev dependency first
