"""T037: Unit tests (RED) for update_batch_status Lambda handler."""
import json
...
class TestUpdateBatchStatusHandler:
    def test_open_to_closed_succeeds(self, tables):
        from src.handlers.update_batch_status import handler  # noqa: PLC0415
        response = handler(_event("b-open", "CLOSED"), MagicMock())
        assert response["statusCode"] == 200
        assert json.loads(response["body"])["status"] == "CLOSED"

    def test_closed_to_completed_succeeds(self, tables):
        from src.handlers.update_batch_status import handler  # noqa: PLC0415
        response = handler(_event("b-closed", "COMPLETED"), MagicMock())
        assert response["statusCode"] == 200
        assert json.loads(response["body"])["status"] == "COMPLETED"

    def test_completed_to_any_fails(self, tables):
        from src.handlers.update_batch_status import handler  # noqa: PLC0415
        for target in ("OPEN", "CLOSED"):
            response = handler(_event("b-completed", target), MagicMock())
            assert response["statusCode"] == 400
            assert json.loads(response["body"])["code"] == "INVALID_STATUS_TRANSITION"

    def test_open_to_completed_fails(self, tables):
        from src.handlers.update_batch_status import handler  # noqa: PLC0415
        response = handler(_event("b-open", "COMPLETED"), MagicMock())
        assert response["statusCode"] == 400
        assert json.loads(response["body"])["code"] == "INVALID_STATUS_TRANSITION"

    def test_closed_to_open_fails(self, tables):
        from src.handlers.update_batch_status import handler  # noqa: PLC0415
        response = handler(_event("b-closed", "OPEN"), MagicMock())
        assert response["statusCode"] == 400
        assert json.loads(response["body"])["code"] == "INVALID_STATUS_TRANSITION"
